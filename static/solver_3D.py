...
def computation(mesh, R, cube, T, nu, mu, lmbda, l, N):
        ...
        torsion = as_tensor([
            [ eta[0].dx(0),
              [ eta[0].dx(1),
                [ eta[0].dx(2),
                  \
                  eta[1].dx(0), eta[2].dx(0) ], \
                eta[1].dx(1), eta[2].dx(1) ], \
              eta[1].dx(2), eta[2].dx(2) ] ])
        return torsion
    ...
    # Function Space
    U = VectorFunctionSpace(mesh, "Lagrange", 2) # displacement space
    S = VectorFunctionSpace(mesh, "Lagrange", 1) # micro rotation space
