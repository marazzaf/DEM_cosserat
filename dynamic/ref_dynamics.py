...
# Define mesh
Lx,Ly,Lz = 1., 0.1, 0.04
mesh = BoxMesh(Point(0., 0., 0.), Point(Lx, Ly, Lz), 3, 2, 2) #60, 10, 5)
...
# Mass density
rho = Constant(1.0)
I = Constant(2) #Quelle valeur donner à ca ?
...
E_damp = 0
E_ext = 0
xdmf_file = XDMFFile("flexion.xdmf")
...
for (i, dt) in enumerate(np.diff(time)):
    ...
    E_tot = E_elas+E_kin
    energies[i+1, :] = np.array([E_elas, E_kin, E_ext, E_tot])
