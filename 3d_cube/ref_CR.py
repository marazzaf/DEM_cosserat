...
# Function Space
U = VectorElement("CR", mesh.ufl_cell(), 1) # displacement space
S = VectorElement("CR", mesh.ufl_cell(), 1) # micro rotation space
...
pen = 2*mu
h_avg = 0.5 * (h('-') + h('+'))
a = inner(epsilon_v, sigma_u)*dx + inner(chi_v, m_u)*dx + pen/h_avg * inner(jump(u),jump(v)) * dS + pen*l*l/h_avg * inner(jump(phi),jump(eta)) * dS
L = inner(t, v)*ds(1)
...
file = File('CR_locking_%i_.pvd' % mesh_num)
file << u_h
file << phi_h
