...
n = FacetNormal(mesh)
lhs_nitsche = -inner(dot(2*mu*sym(grad(u)), n), v) * ds - inner(lambda_*div(u), dot(v,n)) * ds - inner(dot(2*mu*sym(grad(v)), n), u) * ds - inner(lambda_*div(v), dot(u,n)) * ds
lhs_nitsche_bis = -inner(dot(2*mu*sym(grad(u)), n), v) * ds - inner(lambda_*div(u), dot(v,n)) * ds + inner(dot(2*mu*sym(grad(v)), n), u) * ds + inner(lambda_*div(v), dot(u,n)) * ds
