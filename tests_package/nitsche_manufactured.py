...
B = 1 #Same question
u_D = Expression(('A*(x[0]*x[0]+x[1]*x[1])','A*(x[0]*x[0]+x[1]*x[1])'), A=A, degree=3)
phi_D = Expression('B*(x[0]-x[1])', B=B, degree=2)
t = Expression(('-(A*2*(a+c)+B*(c-d))','-(A*2*(a+c)+B*(c-d))'), A=A, B=B, a=a, b=b, c=c, d=d, degree = 2)
c = Expression('2*(x[0]-x[1])*(d-c)*(B+A)', A=A, B=B, c=c, d=d, degree = 2)
